...
public abstract class ParallaxLayer {
   ...
   * scrolling. Parallax Scrolling can be performed infinitely in any direction
   * if TileMode is set to <b>TileMode.repeat</b> in that direction. The layer
   * will just keep on repeating itself. Specifying <b>TileMode.single</b> makes
   * the layer render only once without repeating in a direction
   *
   * @author Rahul
   *
   */
   ...
   * height of this layer after which it either repeats itself while rendering
   * or just ceases to render further, depending upon the vertical TileMode (see
  ...
  /**
   * get the Vector2 that contains the parallax scrolling ratio of this layer in
   * x and y direction.
   *
   ...
   *
   * @param batch the batch used for rendering
   * @param x the x position of the lower left corner where rendering should be
   * done
   * @param y the y position of the lower left corner where rendering should be
   * done
   */
